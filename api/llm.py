...
def parse_text_to_query(text: str) -> MarketDataQuery:
    ...
    """
    prompt = f"""
    You are an AI assistant specialized in analyzing market data queries
    (crypto/stocks). Your task is to read the user's request and extract the necessary
    ...
    If a parameter is not explicitly stated, infer it based on the current context.
    For example, "past 3 days" is calculated from the current time.
    """
    ...
    # Although response_schema requires a Pydantic model, response.text returns JSON
    # as text.
